...
export class EventService {
  ...
  async create(organizationUuid: string, dto: CreateEventDto): Promise<EventEntity> {
    ...
    const { eventType, eventLevel, eventFormat, design } = await this.validateEventReferences(
      organizationUuid,
      dto,
    );
  ...
  async updateByUuid(
    ...
    // Handle optional design update
    if (dto.designId !== undefined) {
      if (dto.designId === null || dto.designId === '') {
        // Clear design
        (updateData as unknown as { design_id: number | null }).design_id = null;
      } else {
  ...
  private async validateEventReferences(
    ...
    const result: {
      ...
      eventFormat: EventFormatEntity;
      design?: DesignEntity;
    } = { eventType, eventLevel, eventFormat };

    // If designId is provided, validate it and attach to result
    const designId = (createDto as unknown as CreateEventDto).designId;
    if (designId) {
