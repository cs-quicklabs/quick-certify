...
export function DesignSelectorModal({ isOpen, onClose, onSelect, designsList }: Props) {
  ...
  const filtered = designsList.filter((d: Design) => {
    const matchFilter =
      filter === 'All' || d.type.toLocaleLowerCase() === filter.toLocaleLowerCase();
        ...
        <div className="bg-white rounded-lg shadow-lg border border-default p-4">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="text-lg font-semibold">
                Select the design you’d like to add in this group
              </h3>
              <p className="text-sm text-gray-500 mt-1">
                A group may consist of one certificate and one badge.
              </p>
            </div>
            <button
              onClick={onClose}
              className="text-body bg-gray-200 hover:bg-gray-300 hover:text-heading rounded-full text-sm w-8 h-8 inline-flex justify-center items-center"
            >
              <X size={'18'} />
            </button>
          </div>
          ...
          <div className="mb-4">
            <div className="flex items-center gap-4 mb-3">
              <button
                className={`px-3 py-2 text-sm ${filter === 'All' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-700'}`}
                onClick={() => setFilter('All')}
              >
                All Designs
              </button>
              <button
                className={`px-3 py-2 text-sm ${filter === 'Certificate' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-700'}`}
                onClick={() => setFilter('Certificate')}
              >
                Certificates
              </button>
              <button
                className={`px-3 py-2 text-sm ${filter === 'Badge' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-700'}`}
            ...
            <div className="flex h-20 flex-col items-center rounded-lg mt-12">
              <div className="flex items-center gap-x-2 text-gray-600">
                <span className="text-sm">
                  {'No design is available yet. You can create a new design from the Design page.'}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-end gap-3">
            {filtered.length > 0 ? (
              <button
                ...
                disabled={!selectedId}
                className={`btn-primary ${selectedId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-200 text-gray-500 cursor-not-allowed'}`}
              >
                Add to Group
              </button>
            ) : (
