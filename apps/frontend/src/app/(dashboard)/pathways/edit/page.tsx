...
export default function EditPathwayPage() {
  ...
  // Populate form with existing pathway data
  useEffect(() => {
    if (pathway && !isInitialized) {
      setName(pathway.name || '');
      setDescription((pathway as Record<string, unknown>).description as string || '');
      setBannerUrl((pathway as Record<string, unknown>).bannerUrl as string || null);
      setSelectedCredentials(
        pathway.credentials?.map((c) => ({ uuid: c.uuid, name: c.name, isFinal: false })) ?? [],
      );
      setIsInitialized(true);
    }
  }, [pathway, isInitialized]);
  ...
  if (!pathway && !isLoading) {
    return (
      <main>
        <section className="min-h-screen">
          <div className="py-6 px-4 mx-auto max-w-2xl lg:py-8">
            <p className="text-center text-gray-500">Pathway not found.</p>
          </div>
        </section>
      </main>
    );
    ...
    <main>
      <section className="min-h-screen">
        <div className="py-6 px-4 mx-auto max-w-2xl lg:py-8">
          {/* Heading */}
          <div className="mb-6">
            <h1 className="form-title">Edit Pathway</h1>
            <p className="form-subtitle">
              Update the pathway details and credentials.
            </p>
          </div>
          ...
          >
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              >
                {updatePathway.isPending ? 'Saving...' : 'Save & Activate'}
              </button>
              <button
                type="button"
