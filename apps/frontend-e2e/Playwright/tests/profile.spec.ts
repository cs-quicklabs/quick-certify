...
test.describe('To validate the Profile Settings Functionality', () => {
  ...
  test('P104_To verify successful profile update with valid first name and last name', async ({ page }) => {
    console.log('Starting test: P104_To verify successful profile update with valid first name and last name');
    await profileSettingsPage.openUrl();
    await profileSettingsPage.waitForFormReady();
  ...
  test('P105_To verify profile update with only first name (last name optional)', async ({ page }) => {
    console.log('Starting test: P105_To verify profile update with only first name (last name optional)');
    await profileSettingsPage.openUrl();
    await profileSettingsPage.waitForFormReady();
  ...
  test('P106_To verify first name field validation - required field error', async () => {
    ...
    if (firstNameValue === '') {
      ...
      const isInvalid = await profileSettingsPage.locator_firstNameField.evaluate((el: HTMLInputElement) => {
        return !el.validity.valid;
      });
      // If HTML5 validation doesn't trigger, check for custom error
      if (!isInvalid) {
        // Try to find error message
        const errorVisible = await profileSettingsPage.locator_firstNameError.first().isVisible().catch(() => false);
        if (errorVisible) {
          await profileSettingsPage.validateFirstNameError(profileData.expectedMessages.firstNameRequiredError);
        }
      }
    }
  });
  ...
  test('P107_To verify first name field validation - minimum length error', async () => {
    ...
    await profileSettingsPage.validateFirstNameError(profileData.expectedMessages.firstNameMinLengthError);
  });
  ...
  test('P108_To verify first name field validation - only letters allowed', async () => {
    ...
    // Assert error is visible and contains expected message
    await expect(profileSettingsPage.locator_firstNameError.first()).toBeVisible({ timeout: 5000 });
    await profileSettingsPage.validateFirstNameError(profileData.expectedMessages.firstNameOnlyLettersError);
  });

  test('P109_To verify first name field validation - cannot be only spaces', async () => {
    console.log('Starting test: P109_To verify first name field validation - cannot be only spaces');
    await profileSettingsPage.openUrl();
    await profileSettingsPage.waitForFormReady();
    ...
    // Assert error is visible and contains expected message
    await expect(profileSettingsPage.locator_firstNameError.first()).toBeVisible({ timeout: 5000 });
    await profileSettingsPage.validateFirstNameError(profileData.expectedMessages.firstNameNoSpacesOnlyError);
  });
  ...
  test('P110_To verify last name field validation - only letters allowed', async () => {
    ...
    await profileSettingsPage.validateLastNameError(profileData.expectedMessages.lastNameOnlyLettersError);
  });
  ...
  test('P115_To verify form fields are pre-populated with existing profile data', async () => {
    console.log('Starting test: P115_To verify form fields are pre-populated with existing profile data');
    await profileSettingsPage.openUrl();
    await profileSettingsPage.waitForFormReady();
